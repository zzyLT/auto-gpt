...
OPENAI_EMAIL = "aaaamiaoaaa@gmail.com"#"zuzhuangyan@gmail.com"#
    ...
    for name in list(data['name']):
            ...
            if str(data.loc[data['name'] == name, 'answer'].values[0]) == 'nan':
                ...
                demande = '接下来你将充当物流行业的职业分析师，配合我完成一系列文本分析任务，要求：' \
                         f'针对{name}，从专业素养（如道德责任、安全环保、沟通合作、创业能力等素养点）、通用能力（如通用能力、学习能力、信息能力等能力）、专业技能三个维度进行分析，每个维度拓展提取并分条列出不少于十条重要能力，每条能力说明需要提供10个技能关键词，并不需要描述性语句。' \
                          f'每个重要能力的名称需严格遵守格式要求即“XXX能力”，' \
                          f'并且能力名称不超过8个中文字符。' \
                          f'同时，每条能力的技能关键词要求用中文顿号隔开，并且每两个顿号之间为完整的技能关键词，技能关键词需为动名词结构，并且每个技能关键词长度严格限定在4-6个中文字符之间。'
                ans = interact_gpt(demande)
                ans = ans + '\n' + interact_gpt('继续')
                data.loc[data['name'] == name, 'answer'] = ans
                time.sleep(5)
                print(data.loc[data['name'] == name, 'answer'])
                data.to_csv('../result/answer_detail.csv', index=False)
    exit()
