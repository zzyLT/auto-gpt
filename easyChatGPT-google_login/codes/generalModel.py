...
class ChatClient:
    ...
    def __init__(self, url:str,
        ...
        if url is None:
            raise NotEnoughInformationException("You did not input url")
    ...
    def __login(self, username: str, password: str) -> None:
        ...
        login_button = self.__sleepy_find_element(By.XPATH,
                                                  '//*[@id="__next"]/div[1]/div[1]/div[4]/button[1]')  # self.login_xq)
        login_button.click()
        time.sleep(1)

        # zzy
        google_login = self.__sleepy_find_element(By.XPATH,
                                                  '/html/body/div/main/section/div/div/div/div[4]/form[2]/button')
        google_login.click()
        time.sleep(1)
        # ===========

        # Find email textbox, enter e-mail
        email_box = self.__sleepy_find_element(By.XPATH, '//*[@id="identifierId"]')  # (By.ID, "username")
        ...
        try:
            pass_box = self.__sleepy_find_element(By.XPATH,
                                                  '//*[@id="password"]/div[1]/div/div[1]/input')  # (By.ID, "password")
            pass_box.send_keys(password)
            # Click continue
            continue_button = self.__sleepy_find_element(By.XPATH,
                                                         '//*[@id="passwordNext"]/div/button')  # (By.XPATH, self.continue_xq)
            continue_button.click()
        except:
            print("TAP PASSWORD!!!!!")
            time.sleep(15)
    ...
    def __wait_to_disappear(self, by, query, sleep_duration=1):
        ...
        while True:
            ...
            if not 'xmlns="http://www.w3.org/2000/svg"' in self.browser.page_source:
                break
            time.sleep(sleep_duration)
        return

    def interact(self, question: dict) -> str:
        """Function to get an answer for a question
        question = {'question_label' = BY.blabla,
                    'question_path' = Xpath or others,
                    'prompt': your question,
                    'answer_label' = BY.blabla,
                    'answer' = Xpath or others}

        """
        self.__log(f"Question received: {question['prompt']}, awaiting chatgpt response... ")
        text_area = self.__sleepy_find_element(question['question_label'], question['question_path'])
        for each_line in question['prompt'].split("\n"):
            text_area.send_keys(each_line)
            text_area.send_keys(Keys.SHIFT + Keys.ENTER)
        text_area.send_keys(Keys.RETURN)
        self.__wait_to_disappear(question['disapear_label'], question['disapear_path'])
        # box = self.browser.find_elements(By.CLASS_NAME, self.chatbox_cq)[0]
        answer = self.browser.find_elements(question['answer_label'], question['answer'])[-1]#(By.TAG_NAME,'ol')[-1]#box.find_elements(By.CLASS_NAME, 'ol')[-1] #self.answer_cq
